...
def forward_inference(
    embedding_net, batch_of_crops, labels, device: str
):  # , agg_method='median'):
...
def aggregate_embeddings_plate(
    ...
    """
    assert operation in ["median", "mean"], (
    ...
    assert len(plate_dfr) == ((len(plate_embs) - 1) * len(plate_embs[0])) + len(
        plate_embs[-1]
    )
