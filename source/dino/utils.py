...
class MultiScaleCrop(object):
    ...
    def __call__(self, x, metadata):
        ...
        for _ in range(self.global_crops_number):
            views.append(self.base_transform(self.crop_global(x, th)))
        for _ in range(self.local_crops_number):
            views.append(self.base_transform(self.crop_local(x, th)))
        return views
...
def _load_checkpoint_compat(path, map_location="cpu"):
    """
    PyTorch 2.6 sets torch.load(weights_only=True) by default which can fail
    for older checkpoints using objects like numpy.core.multiarray.scalar.
    This helper tries safe weights-only loading first (with allowlisted globals),
    then falls back to full unpickling if needed and explicitly allowed.
    """
    if _supports_weights_only():
        ...
        try:
            ...
            # Allowlist known safe numpy scalar used in many historical checkpoints
            try:
                import numpy as _np
                ...
                # Final fallback: full unpickling (only if you trust the source)
...
def load_pretrained_dino(
    ...
    if os.path.isfile(pretrained_weights):
        state_dict = _load_checkpoint_compat(pretrained_weights, map_location="cpu")
...
def restart_from_checkpoint(ckp_path, run_variables=None, **kwargs):
    ...
    checkpoint = _load_checkpoint_compat(ckp_path, map_location="cpu")
